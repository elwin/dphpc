...
class PlotManager:
    ...
    def plot_runtime_by_key(self, df: pd.DataFrame, filter_key = 'N', index_key='numprocs', func_key='mean', powers_of_two = True):
        if func_key == 'mean':
            agg_func = np.mean
    ...
    def plot_runtime_with_scatter(self, df: pd.DataFrame, filter_key = 'N', index_key='numprocs', func_key='median', powers_of_two = True):
        if func_key == 'mean':
            agg_func = np.mean
        elif func_key == 'median':
            agg_func = np.median
        ...
        for i in df[filter_key].unique():
            ...
            for col in data_filtered.columns:
                ...
                x, y = self.removeNanFromCols(data_filtered.index, data_filtered[col])
    ...
    def map_colors(self, implementation_types):
    ...
    def removeNanFromCols(self, index, col):
