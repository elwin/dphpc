...
void allreduce_ring_pipeline::compute(
  ...
  for (int i = 0; i < num_procs - 1; ++i) {
    ...
    auto pipeline_chunk_count = (snd_chunk_length / SEG_EL) + 1;
    auto pipeline_remainder = snd_chunk_length % SEG_EL;
    for (int j = 0; j < pipeline_chunk_count; ++j) {
      auto pipeline_chunk_offset = j * SEG_EL;
      auto pipeline_chunk_length = j == pipeline_chunk_count - 1 ? pipeline_remainder : SEG_EL;

      // Send current chunk to next node
      MPI_Request sendRequest;
      mpi_timer(MPI_Isend, current + snd_chunk_offset + pipeline_chunk_offset, pipeline_chunk_length, MPI_DOUBLE,
          destination, 0, comm, &sendRequest);

      // Receive chunk from previous node
      auto recv_chunk = new double[pipeline_chunk_length];
      mpi_timer(MPI_Recv, recv_chunk, pipeline_chunk_length, MPI_DOUBLE, source, 0, comm, MPI_STATUS_IGNORE);
